...
class Douban_Demo_Controller extends Controller {
	...
	{
		$this->_config = Kohana::config('douban.setting');
		if ($this->_config['api_key'] AND $this->_config['api_secret'])
		{
			$this->_douban = Douban::instance($this->_config['api_key'], $this->_config['api_secret']);
		}
		else
		{
...
var_dump($this->_douban->get_user());
		...
		{
			$output .= '你好，请在执行下面操作前通过豆瓣 OAuth 的'.html::anchor('douban_demo/action_verity_oauth', '验证');
		}
		...
		while ($methods->valid())
		{
			...
			if ( ! in_array($action, $ignore))
			{
				...
				$output .= '<li>'.html::anchor($this->_base.'/douban_demo/'.$action, $action).'</li>';
			}
			$methods->next();
		}
		$output .= '</ol>';
	...
	{
		$callback_url = url::site('douban_demo/action_check');
