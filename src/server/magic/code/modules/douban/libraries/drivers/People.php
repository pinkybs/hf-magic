 ...
 */
class People_Driver extends Douban_Core {
	...
	{	
		...
		{
			$result = People_Driver::format($result->to_json());
		}
	...
	public function get_friends($people_id = NULL, $index = 1, $max = 10)
	{
		...
		if ($this->alt == 'json' AND $this->format AND $result->status() == 200)
		{
			$result = People_Driver::get_peoples($result->to_json());
		}
	...
	public static function get_peoples($peoples)
	{
		...
		{
			$result->author = People_Driver::format($peoples['author']);
		}
		...
		if ($peoples['openSearch:totalResults']['$t'] > 0)
		{
			...
			{
				$result->entry[] = People_Driver::format($people);
			}
		}
		else
		{
