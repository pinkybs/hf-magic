...
class Douban_Core {
	...
	{
		$api_key = empty($api_key) ? Kohana::config('douban.setting.api_key') : $api_key;
		$api_secret = empty($api_secret) ? Kohana::config('douban.setting.api_secret') : $api_secret;
	...
	public function login()
	{
		...
		if ($access_token->status() == 200)
		{
			...
			Cookie::set($this->_config['setting']['session_key']['oauth_token'], serialize($access_token), $this->_config['setting']['lifetime']);
			$this->session->set($this->_config['setting']['session_key']['oauth_token'], $access_token);
			// Get oauth user information
			$people = $this->people()->get('me');
			$this->session->set($this->_config['setting']['session_key']['oauth_user'], $people);
	...
	{
		if ($user = $this->session->get($this->_config['setting']['session_key']['oauth_user']))
		{
			return $user;
		}
		else
		{
			...
			{
				// Save the access token into both cookie and session.
				Cookie::set($this->_config['setting']['session_key']['oauth_token'], serialize($token), $this->_config['setting']['lifetime']);
				$this->session->set($this->_config['setting']['session_key']['oauth_token'], $token);
				// Get oauth user information
				$people = $this->people()->get('me');
				$this->session->set($this->_config['setting']['session_key']['oauth_user'], $people);
	...
	{
		$this->session->delete($this->_config['setting']['session_key']['oauth_token']);
	...
	public function __call($name, $arguments)
	{
		if (empty($arguments))
		{
			...
			else
			{
				...
				$interface = ucfirst($name) . '_Driver';
	...
	{
		if ($token = Cookie::get($this->_config['setting']['session_key']['oauth_token']))
		{
			return unserialize($token);
		}
		else if ($token = $this->session->get($this->_config['setting']['session_key']['oauth_token']))
		{
			return $token;
		}
		else
