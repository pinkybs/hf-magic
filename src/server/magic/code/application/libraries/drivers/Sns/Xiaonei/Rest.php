<?php
require_once "XiaoNei_rest.php";

class Sns_Xiaonei_Rest_Driver {
    ...
    function __construct($api_key, $api_secret){
         ...
         * 调用远程接口
         */
        $this->client = new XiaoNei_Rest($api_key, $api_secret);
    ...
    public function redirect($url) {
        ...
        } else if (preg_match('/^https?:\/\/([^\/]*\.)?renren\.com(:\d+)?/i', $url)) {
    ...
    private function signinUrl($returnUrl, $canvas = true){
        return "http://passport.renren.com/passport.php?action=signin&v=1.1"
    ...
    private function addUrl($returnUrl, $nouse = true){
        return "http://w.renren.com/apps/add?v=1.1"
            . "&api_key=" . $this->api_key
            . "&next=" . urlencode($returnUrl)
            . "" ;
    }
