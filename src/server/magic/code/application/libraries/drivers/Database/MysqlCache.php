 ...
 */
class Database_MysqlCache_Driver extends Database_Driver {
	 ...
	 * @param Array $vl
	 */
	public function setWhereArray($vl)
	{
		$this->_where = $vl;
	}
	...
	public function query($sql)
	{
		...
		return new MysqlCache_Result(mysql_query($sql, $this->link), $this->link, $this->db_config['object'], $sql, $this->_where, $this->_table_name, $this->db_config);
	}
...
class MysqlCache_Result extends Database_Result {
	 ...
	 */
	public function __construct($result, $link, $object = TRUE, $sql, $where = array(), $table_name = '', &$db_config = array())
	{
		$this->result = $result;
		$this->_where = $where;
		$this->_timestamp = PEAR::getStaticProperty('_APP', 'timestamp');
		$this->table_name = $table_name;
		
		//创建db_chache
		if (isset($where['role_id'])) {
			$this->db_cache = DbCache::instance($db_config['config_name'], $where['role_id']);
		} else {
			$this->db_cache = DbCache::instance($db_config['config_name']);
		}
	...
	private function getCacheKey()
	{
		if ($this->cache_key === null) {
			$this->cache_key = $this->table_name;//mysql_field_table($this->result, 0);
			foreach ($this->_where as $key => $vl) {
				$this->cache_key .= "_".$key.'_'.$vl;
			}
		}
	...
	private function getCacheKeyForIndex()
	{
		if ($this->cache_key_for_index === null) {
			$this->cache_key_for_index = $this->table_name;//mysql_field_table($this->result, 0);
			foreach ($this->_where as $key => $vl) {
				if ($key != 'role_id' && $key != 'id') {
					continue;
				}
				$this->cache_key_for_index .= "_".$key.'_'.$vl;
			}
		}
		
		return $this->cache_key_for_index;
	}
	...
	public function next()
	{
		//写入内存
		$data = $this->current();
		$this->cache_data[$this->getCacheKeyForIndex().'_id_'.$data[0]['id']] = $data;
		array_push($this->cache_keys, $this->getCacheKeyForIndex().'_id_'.$data[0]['id']);
		
		++$this->current_row;
		
		if ($this->current_row == $this->total_rows) {
			$this->cache->setMulti($this->cache_data);
			$this->cache->set($this->cache_key.'_keys', $this->cache_keys);
		}
	...
	{	
		...
		if (!isset($this->where['role_id'])) {
			$this->cache = Cache::instance('config_db');
		} else {
			$this->cache = Cache::instance('mc_'.$this->where['role_id']%Kohana::config('base.cut_memcached_num'));
		}
		$data = $this->current();
		...
		if (empty($data[0])) {
			...
			//设置cache_keys
			$this->cache->set($this->cache_key.'_keys', array());
		} else {
			$this->cache_data = array();
			//$this->cache_keys = array($this->getCacheKeyForIndex().'_id_'.$data[0]['id']);
			//$this->cache_data[$this->getCacheKeyForIndex().'_id_'.$data[0]['id']] = $data;
		}
	...
	{
		$data = array($this->offsetGet($this->current_row), 0, $this->_timestamp, array());
		if (!$this->cache && $this->table_name != '') {
			$this->cache_key = $this->getCacheKey();
			...
			if (isset($data[0]['id'])) {
				$id = $data[0]['id'];
				$cache_key = $this->cache_key.'_id_'.$id;
			} else {
				$id = $data[0]['role_id'];
			}
			*/
			if (!empty($this->cache_key)) {
				...
				$this->cache->set($this->cache_key.'_keys', array($this->cache_key));
			}
			//TODO 目前这里只支持
		}
		...
		//TODO 待测试
		if (isset($data[0]['id'])) {
			$key = $this->getCacheKeyForIndex().'_id_'.$data[0]['id'];
			if (isset($this->db_cache->data_list[$key])) {
				$data[$key] = $this->db_cache->data_list[$key];
			}
		}
