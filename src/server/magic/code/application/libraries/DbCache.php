...
class DbCache {
	...
	//真正变更的key_data
	public $change_keys_data_list = array();
		...
		if (empty($data) && self::CACHE_ON) { 
			if (!empty($this->role_id)) {
				...
				if ($cache_key == $role_key) {
					$role_data =  $this->cache->get($role_key);
...
//			}
		...
		if (!isset($data)) {
			if (isset($where['id'])) {
				$db_where = array('id' => $where['id']);
			} elseif(isset($where['role_id'])) {
				$db_where = array('role_id' => $where['role_id']);
			} else {
				$db_where = array();
			}
			...
			if ($limit == 0) {
				$result = $this->db->where($db_where)->orderby($order_by)->get($table);
			} else {
				$result = $this->db->where($db_where)->orderby($order_by)->limit($limit)->get($table);
			}
	...
	public function update($table, $data = array(), $where = array(), $increment = false)
	{
		...
		foreach ($data as $key => $vl) {
			$this->change_keys_data_list[$key_cache][$key] = $vl;
		}
	}
			...
			while($try > 0) {
				if (isset($vl->change_data_list[$role_key])) {
					if ($vl->role_cas_flg === true) {
						//进行数据整合
						$role_data = $vl->cache->getCas($role_key, $vl->role_token);
						foreach ($vl->change_keys_data_list[$role_key] as $k => $v) {
							$role_data[0][$k] = $v;
						}
						
						$vl->cache->cas($vl->role_token, $role_key, $role_data);
						//Kohana::log('error', microtime(true)." try".$try);
						if ($vl->cache->getResultCode() == Cache::RES_SUCCESS) {
							$ok = true;
							//Kohana::log('error', microtime(true)." try".$try.var_export($vl->change_keys_data_list[$role_key], true));
