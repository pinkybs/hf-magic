 ...
 */
class Cache_Tt_Driver extends Cache_Driver {
	...
	public function __construct()
	{
		...
		$this->flags = Kohana::config('cache_tt.compression') ? MEMCACHE_COMPRESSED : 0;

		$servers = Kohana::config('cache_tt.servers');

		foreach ($servers as $server)
		{
			// Make sure all required keys are set
			$server += array('host' => '127.0.0.1', 'port' => 11111, 'persistent' => FALSE);
	...
	{
		count($tags) and Kohana::log('error', 'Cache: Tags are unsupported by the tt driver');
