...
function open_flash_chart_object( $width, $height, $url, $use_swfobject=true, $base='' )
{
    ...
    //
    $aa = _ofc( $width, $height, $url, $use_swfobject, $base );
    echo $aa;
}
...
function _ofc( $width, $height, $url, $use_swfobject, $base )
{
	...
	*/
	if( $use_swfobject )
    {
    	$out[] = '<script type="text/javascript" src="' . $base . 'js/swfobject.js"></script>';
    	$out[] = '<script type="text/javascript">';
    	$out[] = 'swfobject.embedSWF("' . $base . 'open-flash-chart.swf", "myflash", "' . $width . '", "' . $height . '", "9.0.0");';
    	$out[] = '</script>';
    	
    	$out[] = '<div id="myflash">';
    	$out[] = 'This content requires Adobe Flash Player version 9 and above. <a href="http://www.doocom.cn/soft/" target="_blank">Click here to install Flash Player.</a>powered by:<a href="http://www.doocom.cn/" target="_blank">Doocom.cn</a>';
    	//$out[] = '<a href="http://www.doocom.cn/soft/" target="_blank">This content requires Adobe Flash Player version 9 and above. </a><a href="http://www.doocom.cn/" target="_blank">Doocom.cn</a>';
    	$out[] = '</div>';
    }
