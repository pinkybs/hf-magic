...
//		private var _heuristic:Function = euclidian;
		...
		public function findPath(grid:Grid,mustGo:Boolean=false):Boolean
		{
			_grid = grid;
			_open = new Array();
			_closed = new Array();
			...
			if (mustGo) 
			{
				tmpw = _endNode.walkable;
				_endNode.walkable = true;
			}
			...
			var out:Boolean = search();
			if (mustGo) 
			{
				_endNode.walkable = tmpw;
			}
			return out;
		}
		...
		public function search():Boolean
		{
			...
			while(node != _endNode)
			{
				...
				if(_open.length == 0)
				{
					//trace("no path found",_startNode,_endNode);
					return false;
				}
